  ...
  kintone.events.on('app.record.index.show', function(event) {
    ...
    $('#'+sync_kintone.id).on('click', function(){
      ...
      kintone.api(kintone.api.url('/k/v1/records.json', true), 'GET', getReqBody).then(function(resp){
        console.log(resp);
        // 会員情報関連
        var putBody_member={ 'app': sysID.ASS.app.aim, 'records': [] };
        var postBody_member={ 'app': sysID.ASS.app.aim, 'records': [] };
        // シリアル情報関連
        var putBody_sNum={ 'app': sysID.DIPM.app.sn, 'records': [] };
        
        for (var ri in resp.records){
          var mBody={};
          if(resp.records[ri].info_status.value=='new'){
            mBody={
              'member_id': {'value': resp.records[ri].member_id.value },
              'member_type': {'value': resp.records[ri].member_type.value },
              'application_datetime': {'value': resp.records[ri].application_datetime.value },
              'application_type': {'value': resp.records[ri].application_type.value }
            };
            postBody_member.records.push(mBody);
          }else if(resp.records[ri].info_status.value=='update'){
            mBody={
              'updateKey': { 'field': 'member_id', 'value': resp.records[ri].member_id.value },
              'record': {
                'member_type': { 'value': resp.records[ri].member_type.value },
                'application_datetime': { 'value': resp.records[ri].member_type.value },
                'application_type': { 'value': resp.records[ri].member_type.value }
              }
            };
            ...
            for (var i in shipTable){
              ...
              for(var y in sNums){
                var snRecord={
                  ...
                  'record':{
                    'member_id':{'value':memID},
                    'application_datetime':{'value':appDay},
